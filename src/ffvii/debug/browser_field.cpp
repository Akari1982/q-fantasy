...
void BrowserField()
{
    ...
    if (ImGui::Begin("Field Browser", &g_browser_field, ImGuiWindowFlags_NoCollapse))
    {
        ...
        {
            if (ImGui::TreeNode(("Actor " + std::format("{:02x} ", i) + std::string((char*)&l_dat_file[ate_addr + 0x20 + i * 0x8])).c_str()))
            {
                ParseFieldEvents(i);

                ImGui::Text("Вложенный элемент 2");
                
                // Можно создавать вложенные деревья
                if (ImGui::TreeNode("Вложенное дерево"))
                {
                    ImGui::BulletText("Пункт А");
                    ImGui::TreePop(); // Закрываем вложенный узел
                }

                ImGui::TreePop(); // Закрываем основной узел
            }
        }
        ImGui::Separator();
