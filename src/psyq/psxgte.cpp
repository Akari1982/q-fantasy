...
void setCopReg(int cop, int reg, DVECTOR value)
{
...
void copFunction(int cop, u32 function)
{
	...
	{
        case 0x1: RTPS(function); return;
        case 0x6: NCLIP(function); return;
        case 0xC: OP(function); return;
        case 0x12: MVMVA(function); return;
        case 0x28: SQR(function); return;
        case 0x2D: AVSZ3(function); return;
...
{
    setCopReg(2, COP2D_XY2, DVECTOR(pVertices0->vx, pVertices0->vy));
    setCopReg(2, COP2D_Z2, DVECTOR(pVertices0->vz, 0));
}

void gte_ldv0(u8* pVertices)
{
    setCopReg(2, COP2D_XY0, DVECTOR(READ_LE_S16(pVertices), READ_LE_S16(pVertices + 2)));
    setCopReg(2, COP2D_Z0, DVECTOR(READ_LE_S16(pVertices + 4), 0));
}
...
void gte_ldVZ2(s16* pInput) {
    setCopReg(2, COP2D_Z2, DVECTOR(*pInput, 0));
}
...
{
	setCopReg(2, COP2D_XY0, DVECTOR(READ_LE_S16(pVertices0), READ_LE_S16(pVertices0 + 2)));
	setCopReg(2, COP2D_Z0, DVECTOR(READ_LE_S16(pVertices0 + 4), 0));

	setCopReg(2, COP2D_XY1, DVECTOR(READ_LE_S16(pVertices1), READ_LE_S16(pVertices1 + 2)));
	setCopReg(2, COP2D_Z1, DVECTOR(READ_LE_S16(pVertices1 + 4), 0));

	setCopReg(2, COP2D_XY2, DVECTOR(READ_LE_S16(pVertices2), READ_LE_S16(pVertices2 + 2)));
	setCopReg(2, COP2D_Z2, DVECTOR(READ_LE_S16(pVertices2 + 4), 0));
}
...
void gte_ldv3(const SVECTOR* pVertices0, const SVECTOR* pVertices1, const SVECTOR* pVertices2)
{
	...
	setCopReg(2, COP2D_XY1, DVECTOR(pVertices1->vx, pVertices1->vy));
	setCopReg(2, COP2D_Z1, DVECTOR(pVertices1->vz, 0));

	setCopReg(2, COP2D_XY2, DVECTOR(pVertices2->vx, pVertices2->vy));
	setCopReg(2, COP2D_Z2, DVECTOR(pVertices2->vz, 0));
}

void gte_ldlvl(VECTOR* pInput) {
    setCopReg(2, COP2D_IR1, DVECTOR(pInput->vx, 0));
    setCopReg(2, COP2D_IR2, DVECTOR(pInput->vy, 0));
    setCopReg(2, COP2D_IR3, DVECTOR(pInput->vz, 0));
}
...
void gte_stsxy3(DVECTOR* xy0, DVECTOR* xy1, DVECTOR* xy2)
{
	*xy0 = DVECTOR::fromS32(getCopReg(2, 0x6000));
	*xy1 = DVECTOR::fromS32(getCopReg(2, 0x6800));
	*xy2 = DVECTOR::fromS32(getCopReg(2, 0x7000));
}

void gte_stsxy2(DVECTOR* xy)
{
	*xy = DVECTOR::fromS32(getCopReg(2, 0x7000));
}

void gte_stsxy(DVECTOR* xy)
{
    *xy = DVECTOR::fromS32(getCopReg(2, 0x7000));
}
